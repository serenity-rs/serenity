...
impl ChannelId {
    ...
    pub fn broadcast_typing(self, http: impl AsRef<Http>) -> Result<()> {
        http.as_ref().broadcast_typing(self.0)
    }
    ...
    pub fn delete(self, http: impl AsRef<Http>) -> Result<Channel> {
        http.as_ref().delete_channel(self.0)
    }
    ...
    where
        ...
        R: Into<ReactionType>,
    {
        self._delete_reaction(&http, message_id.into(), user_id, &reaction_type.into())
    }
        ...
        F: FnOnce(&mut EditMessage) -> &mut EditMessage,
    {
        ...
        http.as_ref()
            .edit_message(self.0, message_id.0, &Value::Object(map))
    }
    ...
    pub fn invites(self, http: impl AsRef<Http>) -> Result<Vec<RichInvite>> {
        http.as_ref().get_channel_invites(self.0)
    }
    ...
    pub fn say(self, http: impl AsRef<Http>, content: impl std::fmt::Display) -> Result<Message> {
        self.send_message(&http, |m| m.content(content))
    }
    ...
    pub fn webhooks(self, http: impl AsRef<Http>) -> Result<Vec<Webhook>> {
        http.as_ref().get_channel_webhooks(self.0)
    }
}
