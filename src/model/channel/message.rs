...
impl Message {
    ...
    /// [Manage Messages]: ../permissions/struct.Permissions.html#associatedconstant.MANAGE_MESSAGES
    #[cfg(feature = "http")]
    pub async fn delete_reaction_emoji<R: Into<ReactionType>>(&self, cache_http: impl CacheHttp, reaction_type: R) -> Result<()> {
        #[cfg(feature = "cache")]
        {
            if let Some(cache) = cache_http.cache() {
                let req = Permissions::MANAGE_MESSAGES;
    ...
    pub async fn edit<F>(&mut self, cache_http: impl CacheHttp, f: F) -> Result<()>
        ...
        match cache_http.http().edit_message(self.channel_id.0, self.id.0, &Value::Object(map)).await {
            Ok(edited) => {
    ...
    pub async fn suppress_embeds(&mut self, cache_http: impl CacheHttp) -> Result<()> {
        ...
        match cache_http.http()
            .edit_message(self.channel_id.0, self.id.0, &Value::Object(map)).await
        {
            Ok(edited) => {
                mem::replace(self, edited);
                Ok(())
    ...
    pub async fn author_nick(&self, cache_http: impl CacheHttp) -> Option<String> {
        match self.guild_id.as_ref() {
            Some(guild_id) => self.author.nick_in(cache_http, *guild_id).await,
            None => None,
        }
    }
