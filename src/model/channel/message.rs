...
impl Message {
    ...
    async fn _delete_own_reaction(
        ...
        cache_http.http().delete_own_reaction(self.channel_id.0, self.id.0, &reaction_type).await?;
    ...
    async fn _delete_user_reaction(
        ...
        #[cfg(feature = "cache")]
        {
            if let Some(cache) = cache_http.cache() {
                ...
                current_user_id = Some(cache.current_user_id());
            }
        }

        cache_http.http().delete_user_reaction(self.channel_id.0, self.id.0, &reaction_type, user_id).await?;
