...
pub trait EventHandler: Send + Sync {
    ...
    /// ## Warning
    ///
    /// This will run synchronously on every event in the dispatch loop of the shard that is
    /// receiving the event. If your filter code takes too long, it may delay other events from
    /// being dispatched in a timely manner. It is recommended to keep the runtime complexity of
    /// the filter code low to avoid unnecessarily blocking your bot.
...
pub enum FullEvent {
    ...
    ///
    /// Provides the message update data, as well as the old message if cache feature is enabled
    ...
    ///
    /// Provides the presence's new data, as well as the old presence data if the cache feature is
    /// enabled and the data is available.
    ///
