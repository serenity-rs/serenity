...
mod model {
    ...
    fn gen_member() -> Member {
        let dt : DateTime<FixedOffset> = FixedOffset::east(5 * 3600).ymd(2016, 11, 08)
                                          .and_hms(0, 0, 0);
        let vec1 = Vec::new();
    ...
    fn member_named_username() {
        assert_eq!( gen().member_named(&"test#1432".to_string()).unwrap().display_name(), 
    ...
    fn member_named_nickname() {
        assert_eq!( gen().member_named(&"aaaa".to_string()).unwrap().display_name(), 
        gen_member().display_name() );
    }
